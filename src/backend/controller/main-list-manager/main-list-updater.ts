...
export default class MainListEntryUpdater {
    ...
    public async updateSingleProviderData(provider: ProviderLocalDataWithSeasonInfo, series: Series): Promise<void> {
        const oldProvider = series.getOneProviderLocalDataWithSeasonInfoByProviderName(
            provider.providerLocalData.provider
        )
        series.addProviderDatasWithSeasonInfos(provider)
        if (
            oldProvider?.providerLocalData.getAllDetailedEpisodes().length !==
            provider.providerLocalData.getAllDetailedEpisodes().length
        ) {
