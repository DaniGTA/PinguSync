...
export default class ProviderList extends ProviderLoader {
    ...
    public static getAllProviderLists(): ExternalProvider[] {
        return [...this.getInfoProviderList(), ...this.getListProviderList(), ...this.getMappingProviderList()]
    }
     ...
     * @param localdata
     */
    public static getProviderInstanceByLocalData(
        localdata: ProviderLocalData | ProviderLocalDataWithSeasonInfo
    ...
    public static getProviderNameByClass(providerClass: new () => ExternalProvider): string {
        ...
        throw new Error('Failed to get provider name by class for provider: ' + providerClass.name)
    }
    /**
     * Get the provider name value from a ExternalProvider instance.
     * @param providerInstance a ExternalProvider instance
     * @returns the name value of the instance.
     */
    public static getProviderNameByInstance(providerInstance: ExternalProvider): string {
        if (!this.loadedInfoProvider || !this.loadedListProvider || !this.loadedMappingProvider) {
            this.getAllProviderLists()
        }
        if (providerInstance?.providerName) {
            return providerInstance.providerName
        }
        throw new Error('Failed to get provider name by instance for provider: unknown')
    }
    ...
    public static getProviderInstanceByClass<T extends ExternalProvider>(providerClass: new () => T): T {
        ...
        throw new Error(
            'Failed to get provider instance by class for provider: ' + providerClass.name + ' (' + providerName + ')'
        )
    }
}
