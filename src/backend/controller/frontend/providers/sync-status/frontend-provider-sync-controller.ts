...
export default class FrontendProviderSyncController {
    ...
    private init(): void {
        this.com.on(chOnce.GetSyncStatusOfProviderFromASeries, async (x: GetSyncStatus) => this.com.send(chOnce.GetSyncStatusOfProviderFromASeries + x.providerName, await this.getSyncStatusOfProviderFromASeries(x)));
        this.com.on(chListener.OnSyncEpisodeOfSeriesRequest, async (x: FrontendSyncEpisodes) => await this.syncEpisodeOfProvider(x));
    }
