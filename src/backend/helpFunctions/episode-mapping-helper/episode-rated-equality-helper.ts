...
export default class EpisodeRatedEqualityHelper {
    ...
    private targetBindingPools: readonly EpisodeBindingPool[]

    constructor(targetBindingPool: readonly EpisodeBindingPool[], allBindingPools: EpisodeBindingPool[]) {
        this.allBindingPools = allBindingPools
        this.targetBindingPools = targetBindingPool
    }

    public getRatedEqualityFromTwoProviders(
        packageA: ProviderAndSeriesPackage,
        packageB: ProviderAndSeriesPackage,
        season?: Season
    ): EpisodeRatedEqualityContainer[] {
        const providerA = packageA.provider
        const providerB = packageB.provider

        if (
            providerA.seasonTarget?.seasonPart !== undefined &&
            ...
            throw new Error(
                `Cant get rated equality between ${providerA.providerLocalData.provider} and ${providerB.providerLocalData.provider}`
            )
        }

        if (
            providerB.providerLocalData.getAllDetailedEpisodes().length !== 0 &&
            providerA.providerLocalData.getAllDetailedEpisodes().length !== 0
        ) {
    ...
    private getAllRelevantEpisodes(
        provider: ProviderLocalDataWithSeasonInfo,
        otherProvider: ProviderLocalDataWithSeasonInfo
    ) {
        ...
        if (
            Math.abs(aNumberOfEpisodes - bNumberOfEpisodes.length) <=
            EpisodeRatedEqualityHelper.EPISODE_NUMBER_SEASON_TOLERANCE
        ) {
        ...
        for (const detailedEpA of provider.providerLocalData.getAllDetailedEpisodes()) {
            ...
            if (!this.isSameProviderNameInMapping(detailedEpA, otherProvider.providerLocalData, this.allBindingPools)) {
                if (lastSuccessFullIndex < otherProvider.providerLocalData.getAllDetailedEpisodes().length) {
                    for (
                        let index = lastSuccessFullIndex;
                        index < otherProvider.providerLocalData.getAllDetailedEpisodes().length;
                        index++
                    ) {
                        ...
                        if (
                            !this.isSameProviderNameInMapping(
                                detailedEpB,
                                provider.providerLocalData,
                                this.allBindingPools
                            )
                        ) {
                            result.push(detailedEpA)
                            if (detailedEpA.isEpisodeNumberARealNumber() && detailedEpB.isEpisodeNumberARealNumber()) {
                                if (
                                    detailedEpB.type !== EpisodeType.SPECIAL &&
                                    detailedEpA.type !== EpisodeType.SPECIAL
                                ) {
                                    diff = (detailedEpA.getEpNrAsNr() ?? 0) - (detailedEpB.getEpNrAsNr() ?? 0)
                                }
                                lastSuccessFullIndex = index + 1
    ...
    private performRatingEqualityOfEpisodes(
        ...
        for (const detailedEpA of allRelevantResultA.result) {
            for (let index = fastCheck; index < allRelevantResultB.result.length; index++) {
                ...
                let result = this.getRatedEqualityContainer(
                    detailedEpA,
                    detailedEpB,
                    providerA,
                    providerB,
                ...
                if (result !== undefined) {
                    if (
                        result.result.isAbsolute === AbsoluteResult.ABSOLUTE_TRUE ||
                        (result.result.matchAble !== 0 &&
                    ...
                    if (result.result.isAbsolute === AbsoluteResult.ABSOLUTE_TRUE) {
                        ...
                        if (oldDiff !== providerAEpDiff) {
                            const resultWithNewDiff = this.getRatedEqualityContainer(
                                detailedEpA,
                                detailedEpB,
                                providerA,
                                providerB,
     ...
     * @param episodeDiff
     */
    private getRatedEqualityContainer(
        detailedEpA: Episode,
        detailedEpB: Episode,
        providerAWithSeason: ProviderLocalDataWithSeasonInfo,
        providerBWithSeason: ProviderLocalDataWithSeasonInfo,
        season: Season | undefined,
        currentBindingPool: readonly EpisodeBindingPool[],
        episodeDiff: number
    ) {
        ...
        if (result.matches !== 0) {
            ...
            if (
                !this.episodeIsAlreadyMappedToProvider(detailedEpA, providerB, currentBindingPool) &&
                ...
                if (
                    this.isEpisodeAlreadyMappedToEpisode(detailedEpA, detailedEpB, currentBindingPool) &&
    ...
    private episodeIsAlreadyMappedToProvider(
        episode: Episode,
        provider: ProviderLocalData,
        currentBindingPool: readonly EpisodeBindingPool[]
    ): boolean {
        ...
        for (const episodeMapping of list) {
            if (
                episodeMapping.provider === provider.provider &&
    ...
    private isEpisodeAlreadyMappedToEpisode(
        episode: Episode,
        possibleMappedEpisode: Episode,
        current: readonly EpisodeBindingPool[]
    ): boolean {
    ...
    private isSameProviderNameInMapping(
        episode: Episode,
        providerB: ProviderLocalData,
        bindingPools: EpisodeBindingPool[]
    ): boolean {
        if (bindingPools.length !== 0 && episode.provider !== providerB.provider) {
            const list = EpisodeBindingPoolHelper.getAllBindedEpisodesOfEpisode(
                this.getAllEpsidoeBindingPoolsThatAreRelated(),
                episode
            )
