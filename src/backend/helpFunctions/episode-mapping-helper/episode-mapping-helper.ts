...
export default class EpisodeMappingHelper {
    ...
    constructor(series: Series) {
        ...
        this.allProcessedSeries.push(this.currentSeries);
    }
    ...
    private async createMappingForSequels(season: Season, currentPackages: ProviderAndSeriesPackage[]) {
        ...
        if (sequel) {
            ...
            do {
                this.allProcessedSeries.push(sequel);
    ...
    private getMaxEpisodeShiftingDifference(providerA: ProviderLocalData, providerB: ProviderLocalData): number {
        ...
        for (let index = 0; index < lengthA; index++) {
            ...
            if (providerB.detailEpisodeInfo[index] !== undefined &&
                this.isEpisodeAlreadyMappedToEpisode(episodeA, providerB.detailEpisodeInfo[index], this.currentSeries.episodeBindingPools)) {
