...
export default class SeasonFindHelper {
     ...
     */
    public static async searchSeasonValuePrequelTrace(
        series: Series,
        seriesList?: Series[] | readonly Series[]
    ): Promise<SearchSeasonValueResult> {
        ...
        if (series.isAnyPrequelPresent() && seriesList) {
            ...
            if (searchResult.relationExistButNotFounded) {
                return new SearchSeasonValueResult(
                    new Season([-2]),
                    'PrequelTraceNotAvaible',
                    SeasonError.SEASON_TRACING_CAN_BE_COMPLETED_LATER,
                    searchResult
                )
            } else {
                ...
                while (prequel && alreadyCheckedPrequels.findIndex(x => x === prequel?.id) === -1) {
                    ...
                    if (mediaTypePrequel === mediaTypeSeries) {
                        ...
                        if (prequelSeasonNumber && prequelSeason.seasonError === SeasonError.NONE) {
                            return new SearchSeasonValueResult(
                                new Season([prequelSeasonNumber + searchCount]),
                                'PrequelTrace'
                            )
                        }
                    }
                    if (prequel.isAnyPrequelPresent()) {
                        ...
                        return new SearchSeasonValueResult(
                            new Season([searchCount]),
                            'PrequelTrace - nomore prequel present'
                        )
                    } else {
                        prequel = null
                    }
     ...
     * @param seriesList a list where the relation should be.
     */
    public static async searchSeasonValueSequelTrace(
        series: Series,
        seriesList?: Series[] | readonly Series[]
    ): Promise<SearchSeasonValueResult> {
                ...
                while (sequel && alreadyCheckedSequels.findIndex(x => x === sequel?.id) === -1) {
                    ...
                    if (sequel.getMediaType() === series.getMediaType()) {
                        ...
                        if (sequelSeasonNumber && sequelSeason.seasonError === SeasonError.NONE) {
                            return new SearchSeasonValueResult(
                                new Season([sequelSeasonNumber - searchCount]),
                                'SequelTrace'
                            )
                        }
                    }
    ...
    public static async prepareSearchSeasonValue(
        series: Series,
        ...
        if (this.isSeriesCurrentlyInTheSeasonProcess(series)) {
            return new SearchSeasonValueResult(
                new Season(undefined, undefined, SeasonError.CANT_GET_SEASON),
                'None',
                SeasonError.CANT_GET_SEASON
            )
        }
        ...
        try {
            ...
            SeasonFindHelper.currentSeriesSeasonRequest = listHelper.removeEntrys(
                SeasonFindHelper.currentSeriesSeasonRequest,
                series
            )
            return result
        } catch (err) {
            ...
            for (const prequelId of entry.prequelIds) {
                if (prequelId !== undefined && prequelId !== null) {
                    ...
                    if (entry instanceof ListProviderLocalData) {
                        const newProvider = new ListProviderLocalData(
                            prequelId,
                            ProviderList.getProviderInstanceByProviderName(entry.provider) as ListProvider
                        )
                        ...
                        const newProvider = new InfoProviderLocalData(
                            prequelId,
                            ProviderList.getProviderInstanceByProviderName(entry.provider) as InfoProvider
                        )
                        newProvider.infoStatus = ProviderInfoStatus.ONLY_ID
                        newProvider.sequelIds.push(entry.id as number)
     ...
     */
    private static async searchSeasonValue(
        series: Series,
        ...
        if (SeasonSearchModeHelper.canPerformATitleSearch(searchMode)) {
            ...
            if (
                numberFromName &&
                numberFromName.seasonNumber &&
                numberFromName.absoluteResult === AbsoluteResult.ABSOLUTE_TRUE
            ) {
