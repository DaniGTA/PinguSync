...
export default class ProviderInfoHelper {
    public static async requestAllInfoProviderInfos(
        ...
        for (const infoProvider of infoProvidersThatNeedUpdates) {
            try {
                if (
                    seasonAware &&
                    !hasAlreadyFullEpisodeInfo &&
                    this.shouldUpdateProviderLocalData(infoProvider, tempSeriesCopy)
                ) {
    ...
    private static async getInfoProviderThatNeedUpdates(
        currentProviders: ProviderLocalData[]
    ...
    private static needInfoProviders(currentProviders: ProviderLocalData[]): boolean {
        ...
        const maxSeasonNumber: number = this.getFromAllProviderLocalDataMaxSeasonNumber(
            allNonSeasonAwareProviderLocalDatas
        )
    ...
    private static getFromAllProviderLocalDataMaxSeasonNumber(allProviderLocalData: ProviderLocalData[]): number {
        ...
        for (const nonSeasonAwareProviderLocalData of allProviderLocalData) {
            try {
                const seasonNumber = EpisodeStatsHelper.getMaxSeasonNumberFromEpisodeList(
                    nonSeasonAwareProviderLocalData.getAllDetailedEpisodes()
                )
