...
export default class ProviderListHelper {
     ...
     */
    public static async getAllRelevantListProviders(
        currentProviderData: ProviderLocalData[] = []
    ): Promise<ListProvider[]> {
    ...
    public static async requestAllListProviderInfos(
        series: Series,
        force: boolean,
        ...
        for (const providerThatNeedUpdate of providersThatNeedUpdates) {
            try {
                const providerLocalData = tempSeriesCopy
                    .getListProvidersInfos()
                    .find(x => x.provider === providerThatNeedUpdate.providerName)
                if ((providerLocalData && providerLocalData.infoStatus > target) || !providerLocalData) {
                    const requestResults = await new DownloadProviderLocalDataToTargetHelper(
                        tempSeriesCopy,
                        providerThatNeedUpdate,
    ...
    private static async getProvidersThatNeedUpdates(
        currentProviderData: ProviderLocalData[],
        target: ProviderInfoStatus
        ...
        for (const provider of allProviders) {
            if (relevantList.find(x => x.providerName === provider.providerName)) {
                ...
                if (currentLocalData) {
                    if (
                        currentLocalData.infoStatus > target ||
                        !ProviderHelper.isTheLocalDataStillUpToDate(currentLocalData)
                    ) {
    ...
    private static getInLocalDataListContainsThisProvider(
     ...
     */
    private static sortProvidersThatNeedUpdates(
        a: ExternalProvider,
        b: ExternalProvider,
        currentProviderData: ProviderLocalData[]
    ): number {
