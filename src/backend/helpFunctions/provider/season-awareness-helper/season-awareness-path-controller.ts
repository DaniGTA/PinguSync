...
export default class SeasonAwarenessPathController {
    ...
    public async getProviderLocalDataWithSeason(targetSeason: Season | undefined): Promise<Season | undefined> {
        ...
        if (EpisodeHelper.hasEpisodeNames(this.providerWithAwareness.getAllDetailedEpisodes())) {
            const result = new EpisodeRelationAnalyser(
                this.providerWithoutAwarness.getAllDetailedEpisodes(),
                this.providerWithAwareness.getAllDetailedEpisodes()
            )
    ...
    private async seasonAwarenessChoicePath(
        ...
        if (!targetSeason || this.checkIfTheSeasonIsTheSame(result, targetSeason)) {
            if (result.finalSeasonNumbers !== undefined && result.seasonComplete) {
                ...
                result.minEpisodeNumberOfSeasonHolder !== 1 &&
                (this.pathMode === SeasonAwarenessPath.UNKNOWN || this.pathMode === SeasonAwarenessPath.PREQUEL)
            ) {
                const prequelSeason = await SeasonAwarenessPrequelPathController.processPrequel(
                    this.providerWithAwareness,
                    this.providerWithoutAwarness,
                    new Season(result.finalSeasonNumbers)
                )
                const prequel = prequelSeason.find(
                    x => x.providerLocalData.provider === this.providerWithAwareness.provider
                )

                if (
                    prequel &&
                    SeasonComperator.isSameSeasonNumber(prequel.seasonTarget, new Season(result.finalSeasonNumbers))
                ) {
                    if (targetSeason) {
                ...
                SeasonAwarenessSequelPathController.addSequelTooMainList(
                    this.providerWithAwareness,
                    this.providerWithoutAwarness,
                    result
                )
            ...
            if (this.pathMode === SeasonAwarenessPath.UNKNOWN || this.pathMode === SeasonAwarenessPath.SEQUEL) {
                const sequelResult = await SeasonAwarenessSequelPathController.processSequel(
                    this.providerWithAwareness,
                    this.providerWithoutAwarness,
                    targetSeason
                )
                const sequel = sequelResult.find(
                    x => x.providerLocalData.provider === this.providerWithAwareness.provider
                )
                if (sequel) {
                ...
                if (
                    sequelResult[0].seasonTarget?.seasonPart === undefined ||
                    sequelResult[0].seasonTarget?.seasonPart === 1
                ) {
                    return sequelResult[0].seasonTarget
                } else {
                    return new Season(
                        sequelResult[0].seasonTarget.seasonNumbers,
                        1 - sequelResult[0].seasonTarget.seasonPart
                    )
                }
            }
        }
