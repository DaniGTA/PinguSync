...
export default class ProviderComperator {
    ...
    public static async simpleProviderSameIdAndSameSeasonCheckWithSeries(
        a: ProviderLocalData[],
        b: ProviderLocalData[],
        aSeries?: Series,
        bSeries?: Series
                ...
                if (aProvider.provider === bProvider.provider) {
                    ...
                    try {
                        if (ProviderList.getProviderInstanceByLocalData(aProvider).hasUniqueIdForSeasons) {
                            ...
                            if (
                                ProviderComperator.simpleProviderIdCheck(aProvider.id, bProvider.id) &&
    ...
    public static isBindingAndProviderLocalDataWithSeasonTheSame(
        pws: ProviderLocalDataWithSeasonInfo,
        binding: LocalDataBind
    ): boolean {
    ...
    private compareProviderAWithSameProvider(
        providerA: ProviderLocalData,
        ...providers: ProviderLocalData[]
    ): ComperatorResult {
    ...
    private compareProviderAWithProviderB(
        providerA: ProviderLocalData,
        providerB: ProviderLocalData
    ): ComperatorResult {
        logger.debug(
            `[compareProviderAWithProviderB] Comparing season between provider: ${providerA.provider} (id:${providerA.id}) and ${providerB.provider} (id: ${providerB.id})`
        )
        ...
        if (ProviderComperator.simpleProviderIdCheck(providerA.id, providerB.id)) {
            ...
            try {
                if (ProviderList.getProviderInstanceByLocalData(providerA).hasUniqueIdForSeasons) {
                    ...
                    seasonHelper.isSeasonUndefined(providerASeason) &&
                    seasonHelper.isSeasonUndefined(providerBSeason)
                ) {
                    if (
                        !seasonHelper.isSeasonUndefined(this.aSeriesSeason) &&
                        SeasonComperator.isSameSeason(this.aSeriesSeason, this.bSeriesSeason)
                    ) {
                        comperatorResult.isAbsolute = AbsoluteResult.ABSOLUTE_TRUE
                    } else if (
                        !seasonHelper.isSeasonUndefined(this.aSeriesSeason) &&
                        !seasonHelper.isSeasonUndefined(this.bSeriesSeason)
                    ) {
            ...
            if (
                providerA.infoStatus !== ProviderInfoStatus.ONLY_ID &&
                providerB.infoStatus !== ProviderInfoStatus.ONLY_ID
            ) {
                comperatorResult.isAbsolute = AbsoluteResult.ABSOLUTE_FALSE
            } else {
                comperatorResult.isAbsolute = AbsoluteResult.NOT_ABSOLUTE_TRUE
