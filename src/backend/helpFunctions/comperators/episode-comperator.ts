...
export default class EpisodeComperator {
    public static compareEpisodes(a: Series, b: Series): ComperatorResult {
        ...
        try {
            ...
            if (allAEpisodes.length !== 0 && allBEpisodes.length !== 0) {
                result.matchAble += 2
                if (
                    allAEpisodes.findIndex(valueA => allBEpisodes.findIndex(valueB => valueB === valueA) !== -1) !== -1
                ) {
                    result.matches += 2
                }
            }
     ...
     */
    public static compareDetailedEpisode(
        aEpisode: Episode,
        bEpsiode: Episode,
        providerASeason?: Season,
        providerBSeason?: Season,
        season?: Season,
        upshift = 0
    ): ComperatorResult {
        const result = new ComperatorResult()
        if (aEpisode.id === bEpsiode.id) {
            ...
            if (aEpisode.provider === bEpsiode.provider) {
                result.matchAble++
                if (
                    aEpisode.providerEpisodeId === bEpsiode.providerEpisodeId &&
                    aEpisode.providerEpisodeId !== undefined
                ) {
                    result.isAbsolute = AbsoluteResult.ABSOLUTE_TRUE
                    result.matches++
                    return result
        ...
        result.matchAble += 2
        if (
            aEpisode.episodeNumber ===
            (!isNaN((bEpsiode.episodeNumber as unknown) as number) && (bEpsiode.episodeNumber as number) + upshift)
        ) {
            result.matches += 2
     ...
     */
    public static isEpisodeSameSeason(
        aEpisode: Episode,
        bEpisode: Episode,
        providerASeason?: Season,
        providerBSeason?: Season,
        ...
        if (
            aSeason !== undefined &&
            !aSeason.isSeasonUndefined() &&
            bSeason !== undefined &&
            !bSeason.isSeasonUndefined()
        ) {
    ...
    public static isSameEpisodeMapping(aEpisodeMapping: EpisodeMapping, bEpisodeMapping: EpisodeMapping): boolean {
        if (aEpisodeMapping.id === bEpisodeMapping.id) {
            ...
            if (
                this.isSameEpisodeNumber(aEpisodeMapping.episodeNumber, bEpisodeMapping.episodeNumber) &&
                SeasonComperator.isSameSeason(aEpisodeMapping.season, bEpisodeMapping.season)
            ) {
                return true
            } else if (
                aEpisodeMapping.providerEpisodeId !== undefined &&
                aEpisodeMapping.providerEpisodeId === bEpisodeMapping.providerEpisodeId
            ) {
                return true
            }
        }
    ...
    public static isSameEpisodeMappingToEpisode(aEpisodeMapping: EpisodeMapping, aEpisode: Episode): boolean {
        if (aEpisodeMapping.id === aEpisode.id) {
            ...
            // tslint:disable-next-line: triple-equals
            if (
                aEpisodeMapping.episodeNumber == aEpisode.episodeNumber &&
                SeasonComperator.isSameSeason(aEpisodeMapping.season, aEpisode.season)
            ) {
                return true
            } else if (
                aEpisodeMapping.providerEpisodeId !== undefined &&
                aEpisodeMapping.providerEpisodeId === aEpisode.providerEpisodeId
            ) {
                return true
            }
        }
    ...
    public static isSameEpisodeTitle(aEpisode: Episode, bEpsiode: Episode): ComperatorResult {
        const result = new ComperatorResult()
        if (
            (aEpisode.title.length !== 0 || bEpsiode.title.length !== 0) &&
            (aEpisode.provider !== bEpsiode.provider || aEpisode.provider === undefined)
        ) {
            result.matchAble++
    ...
    private static isEpisodeTitleEquals(textA: string, textB: string): boolean {
        ...
        if (textA == textB) {
            return true
        } else if (this.isEpisodeTitleTooUnlikleToBeEquals(textA, textB)) {
            return false
        }
    ...
    private static isEpisodeTitleTooUnlikleToBeEquals(textA: string, textB: string): boolean {
        if (Math.abs(textA.length - textB.length) > 10) {
            return true
        }
        return false
    }
