...
export default class MalConverter {
    public static convertMalListToMultiProviderResult(
        lists: Mal.Common.Paging<Mal.User.AnimeListItem<Mal.Common.WorkBase, Mal.Anime.AnimeListStatusBase>>
    ): MultiProviderResult[] {
    ...
    public static convertSearchResultData(animeInfo: any): ListProviderLocalData {
        ...
        if (animeInfo?.synopsis) lpd.addOverview(new Overview(animeInfo.synopsis, 'en'))
        return lpd
    }
    public static async convertAnimeToProviderData(anime: Jikan.Anime.Anime): Promise<MultiProviderResult> {
        ...
        if ((animeInfo.related as any)['Sequel'])
            lpd.sequelIds.push(...(animeInfo.related as any)['Sequel'].map((x: Jikan.Types.Common.MalUrl) => x.mal_id))
        if ((animeInfo.related as any)['Prequel'])
            lpd.prequelIds.push(
                ...(animeInfo.related as any)['Prequel'].map((x: Jikan.Types.Common.MalUrl) => x.mal_id)
            )
    ...
    private static convertMalEpisodeToEpisode(ep: Jikan.Anime.EpisodeListItem): Episode {
        ...
        episode.title.push(new EpisodeTitle(ep.title, 'unknown'))
        if (ep.title_romanji) episode.title.push(new EpisodeTitle(ep.title_romanji, 'romanji'))
        if (ep.title_japanese) episode.title.push(new EpisodeTitle(ep.title_japanese, 'jap'))
        return episode
    }
    ...
    private static getBackground(info: Jikan.Anime.AnimeInfo): Banner | undefined {
        if (info.background) return new Banner(info.background, ImageSize.LARGE)
    }
    ...
    private static convertMainPicturToCovers(mainPicture: Mal.Common.Picture | null): Cover[] {
        const covers: Cover[] = []
        if (mainPicture) {
            if (mainPicture.large) covers.push(new Cover(mainPicture.large, ImageSize.LARGE))
            if (mainPicture.medium) covers.push(new Cover(mainPicture.medium, ImageSize.MEDIUM))
        }
        return covers
    }
}
