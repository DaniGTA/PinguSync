...
export default class OMDbProvider extends InfoProvider {
    ...
    public async getMoreSeriesInfoByName(
        ...
        try {
            const result = await this.webRequest<SearchResults>(
                `https://www.omdbapi.com/?apikey=${this.getApiSecret() ?? ''}&s=${encodeURI(searchTitle)}`
            )
            if (result.Search) {
    ...
    public async getFullInfoById(provider: InfoProviderLocalData): Promise<MultiProviderResult> {
        if (provider.provider === this.providerName) {
            try {
                const converter = new OMDbConverter()
                const result = await this.webRequest<IdRequestResult>(
                    `https://www.omdbapi.com/?apikey=${this.getApiSecret() ?? ''}&i=${provider.id}`
                )
