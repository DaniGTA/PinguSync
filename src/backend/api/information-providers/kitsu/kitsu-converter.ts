...
export default new (class KitsuConverter {
    public async convertMediaToAnime(
        media: IMedia,
        fullInfo: ProviderInfoStatus = ProviderInfoStatus.FULL_INFO
    ): Promise<MultiProviderResult> {
        logger.debug(
            '[KitsuConverter] Starting converting media to provider local data. MediaInfo: ID: ' +
                media.id +
                ' TYPE:' +
                media.type
        )
        ...
        if (media.episodes && Array.isArray(media.episodes)) {
        ...
        if (media.mappings && Array.isArray(media.mappings)) {
    ...
    public async convertKitsuMappingsToProvider(
    ...
    ): Promise<Array<ProviderLocalData | ProviderLocalDataWithSeasonInfo>> {
        ...
        for (const mapping of mappings) {
            try {
                if (mapping.externalSite === 'anidb') {
                    ...
                    //roviderLocalData.push(localdata);
                    ...
                    mapping.externalSite === 'myanimelist/manga'
                ) {
        ...
        try {
            ...
            if (result) {
                ...
                const localdata = new ProviderLocalDataWithSeasonInfo(
                    new InfoProviderLocalData(idSeason[0], TVDBProvider),
    ...
    public async convertKitsuEpisodesToEpisodes(episodes: IKitsuEpisode[]): Promise<Episode[]> {
        ...
        for (const episode of episodes) {
            const episodeTitles = []
            if (episode.titles) {
                if (Array.isArray(episode.titles)) {
            ...
            if (episode.thumbnail) {
                if (Array.isArray(episode.thumbnail)) {
                    ...
                    episodeThumbnails.push(
                        new EpisodeThumbnail((episode as any).thumbnail.original, ImageSize.ORIGINAL)
                    )
                }
            }
