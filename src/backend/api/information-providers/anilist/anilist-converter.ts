...
export default new (class AniListConverter {
    ...
    public convertListEntryToAnime(entry: Entry, watchStatus: ListType): MultiProviderResult {
        ...
        let providerInfo: ListProviderLocalData = new ListProviderLocalData(
            entry.media.id,
            AniListProvider.getInstance()
        )
    ...
    private fillRelation(providerInfo: ListProviderLocalData, relations: Relation): ListProviderLocalData {
        ...
        const alternatives = listHelper.findAllIndexes(
            relations.edges,
            item => item.relationType === MediaRelation.ALTERNATIVE
        )
