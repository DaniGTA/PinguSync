...
export default new (class TraktConverter {
    public async convertSeasonsToMultiProviderResult(watchedInfo: WatchedInfo): Promise<MultiProviderResult[]> {
        ...
        for (const season of watchedInfo.seasons) {
            const providerInfo: ListProviderLocalData = new ListProviderLocalData(
                watchedInfo.show.ids.trakt,
                TraktProvider.getInstance()
            )
            ...
            result.push(
                new MultiProviderResult(new ProviderLocalDataWithSeasonInfo(providerInfo, new Season([season.number])))
            )
        }
        return result
    }
    ...
    public combineSeasonInfoAndSeasonEpisodeInfo(
        seasonInfo: ITraktShowSeasonInfo[],
        seasonEpisodeInfo: ITraktShowSeasonInfo[]
    ): ITraktShowSeasonInfo[] {
    ...
    public async convertFullShowInfoToLocalData(
        fullShow: FullShowInfo,
        mediaType: MediaType,
        seasonInfo?: ITraktShowSeasonInfo[]
    ...
    public async getDetailedEpisodeInfo(seasonInfos: ITraktShowSeasonInfo[]): Promise<Episode[]> {
        ...
        for (const seasonInfo of seasonInfos) {
            let seasonNumber = seasonInfo.title
                ? TitleHelper.getSeasonNumberBySeasonMarkerInTitle(seasonInfo.title).seasonNumber
                : ''
    ...
    public async convertAnimeToSendRemoveEntryShow(series: Series, removeEpisode: number): Promise<SendEntryUpdate> {
        const currentProvider = series
            .getListProvidersInfos()
            .find(x => x.provider === ProviderNameManager.getProviderName(TraktProvider))
    ...
    public async convertAnimeToSendEntryShow(series: Series, newWatchprogress: number): Promise<SendEntryUpdate> {
        ...
        if (currentProvider && seasonNumbers) {
            ...
            for (let i = 1; i < maxEpisodes; i++) {
                for (
                    let index = 1;
                    index < newWatchprogress + 1 && index < maxEpisodes + 1 && i < newWatchprogress + 1;
                    index++
                ) {
