...
export default class TraktProvider extends ListProvider {
    ...
    public async markEpisodeAsUnwatched(episodes: Episode[]): Promise<void> {
        ...
        await this.traktRequest<Movies>(
            'https://api.trakt.tv/sync/history/remove',
            'POST',
            JSON.stringify(episodeUpdates)
        )
    }
    ...
    public async getMoreSeriesInfoByName(seriesName: string): Promise<MultiProviderResult[]> {
        ...
        const searchResults = await this.traktRequest<TraktSearch[]>(
            'https://api.trakt.tv/search/movie,show?query=' + encodeURI(seriesName)
        )
    ...
    public async getAllSeries(disableCache = false): Promise<MultiProviderResult[]> {
        ...
        if (this.userData.list != null && this.userData.list.length !== 0 && !disableCache) {
            ...
            const data = await this.traktRequest<WatchedInfo[]>(
                'https://api.trakt.tv/users/' + this.userData.userInfo.user.ids.slug + '/watched/shows'
            )
