...
describe('ListController | Combine', () => {
    ...
    function getFilledAnime(
        provider: new () => ListProvider = TestListProvider,
        providerId = -1,
        targetSeason: number | null | undefined = 3
    ): Series {
        ...
        anime.addProviderDatasWithSeasonInfos(
            new ProviderDataWithSeasonInfo(providerLocalData, new Season(targetSeason))
        )
        return anime
    }
    ...
    test('should clean doubled entrys (1/2)', async () => {
        ...
        await MainListManager.addSerieToMainList(x1)
        await MainListManager.addSerieToMainList(x2)
        await MainListManager.addSerieToMainList(x3)
        expect(MainListManager['mainList'].length).toEqual(1)
    })
