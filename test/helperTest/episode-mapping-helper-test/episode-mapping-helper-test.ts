...
describe('Episode mapping | Mapping Only', () => {
    ...
    test('should map same episodes length from 3 providers', async () => {
        ...
        const cProvider = new ListProviderLocalData('1', TestListProvider3)
    ...
    test('should map same episodes length from different providers without detailedEpisodes', async () => {
        const aSeries = new Series()
        ...
        const cProvider = new ListProviderLocalData('1', TestListProvider2)
    ...
    test('should map find unmapped episodes in the sequel', async () => {
        ...
        const cProvider = new InfoProviderLocalData(2, TestInfoProvider)
    ...
    test('should map find unmapped episodes in the sequel of the sequel', async () => {
        /**
        ...
        const dProvider = new InfoProviderLocalData(3, TestInfoProvider)
        dProvider.episodes = 3

        sequelOfSequelOfaSeries.addProviderDatasWithSeasonInfos(
            new ProviderLocalDataWithSeasonInfo(dProvider, new Season([3]))
        )
        sequelOfSequelOfaSeries.addProviderDatasWithSeasonInfos(
            new ProviderLocalDataWithSeasonInfo(bProvider, new Season([3]))
        )
    ...
    describe('sorting episode results', () => {
        test('should sort result container right', async () => {
            ...
            const sorted = await sortHelper.quickSort([ep1erec, ep2erec], async (a, b) =>
                EpisodeRatedEqualityContainerHelper.sortingEpisodeRatedEqualityContainerByResultPoints(a, b)
            )
            const sorted2 = await sortHelper.quickSort([ep2erec, ep1erec], async (a, b) =>
                EpisodeRatedEqualityContainerHelper.sortingEpisodeRatedEqualityContainerByResultPoints(a, b)
            )
