...
describe('Episode mapping | Season Mapping Tests Only', () => {
    ...
    test('should map episode with right season (3/3)', async () => {
        ...
        if (
            ...
            expect(EpisodeBindingPoolHelper.getAllBindedEpisodesOfEpisode(result, bEpisodeInfo1s2)[0].id).toBe(
                aEpisodeInfo1.id
            )
            expect(EpisodeBindingPoolHelper.getAllBindedEpisodesOfEpisode(result, bEpisodeInfo2s2)[0].id).toBe(
                aEpisodeInfo2.id
            )
            expect(EpisodeBindingPoolHelper.getAllBindedEpisodesOfEpisode(result, bEpisodeInfo3s2)[0].id).toBe(
                aEpisodeInfo3.id
            )
        }
    })

    test('should map episode with right season (Provider is in second season)', async () => {
        const aSeries = new Series()
        // tslint:disable-next-line: no-string-literal
        aSeries['cachedSeason'] = new Season([1])
        // A Site
