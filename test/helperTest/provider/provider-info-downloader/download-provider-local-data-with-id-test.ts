...
describe('Download provider local data with provider id', () => {
    ...
    test('should download without problems.', async () => {
        ...
        jest.spyOn(provider, 'getFullInfoById').mockImplementation(
            async () => new MultiProviderResult(new ProviderLocalDataWithSeasonInfo(resultListProvider))
        )

        const result = await DownloadProviderLocalDataWithId.download(
            provider,
            new ListProviderLocalData(1, TestListProvider)
        )
