...
describe('Download provider local data to target info status helper test (download-provider-local-data-to-target-helper.ts)', () => {
    ...
    describe('Testing function: downloadProviderLocalDataUntilTarget()', () => {
        test('should upgrade', async () => {
            ...
            mock.mockImplementationOnce(async () => new MultiProviderResult(firstLocalData)).mockImplementationOnce(
                async () => new MultiProviderResult(secondLocalData)
            )
    ...
    describe('Testing function: canGetTargetIdFromCurrentProvider()', () => {
        test('should be true', () => {
            ...
            const result = new DownloadProviderLocalDataToTargetHelper(
                new Series(),
                currentProvider,
                ProviderInfoStatus.FULL_INFO
            )['canGetTargetIdFromCurrentProvider'](currentProviderLocalData, targetProvider)
