...
describe('Episode binding pool helper tests | Testrun', () => {
    test('should get all binded episode from an episode', () => {
        ...
        const result = EpisodeBindingPoolHelper.getAllBindedEpisodesOfEpisode(
            [episodeBindingPool, episode2BindingPool],
            episode
        )
    ...
    test('should not get season 2 with special season', () => {
        ...
        const specialEpisodeMapping = new EpisodeMapping(
            specialEpisode,
            new ListProviderLocalData(1, TestListProvider2)
        )
    ...
    describe('testing function: isEpisodeAlreadyBinded', () => {
        ...
        test('should find already binded episode', () => {
            const result = EpisodeBindingPoolHelper.isEpisodeAlreadyBinded(
                [episodeBindingPool],
                episode1,
                TestListProvider2.name
            )
            expect(result).toBeTruthy()
        })

        test('should not find already binded episode', () => {
            const result2 = EpisodeBindingPoolHelper.isEpisodeAlreadyBinded([episodeBindingPool], episode1, 'c')
