...
export default class ResponseHelper {
        ...
        const spyGetInfoByNameMock = async (searchTitle: string, season: number): Promise<MultiProviderResult[]> => {
            ...
            if (ResponseHelper.isRequestCached(requestId)) {
                ...
                for (let index = 0; index < resultList.length; index++) {
                    ...
                    response.mainProvider.providerLocalData = ProviderDataListLoader['createProviderLocalDataInstance'](
                        response.mainProvider.providerLocalData
                    )
                    ...
                    for (let index = 0; index < response.subProviders.length; index++) {
                        ...
                        subProvider.providerLocalData = ProviderDataListLoader['createProviderLocalDataInstance'](
                            subProvider.providerLocalData
                        )
    ...
    private static mockGetByIdRequest(provider: new () => ExternalInformationProvider): void {
        ...
        const spyGetInfoByIdMock = jest.fn().mockImplementation(
            async (providerLocalData: ProviderLocalData): Promise<MultiProviderResult> => {
                ...
                if (ResponseHelper.isRequestCached(requestId)) {
                    ...
                    this.cacheNewRequest(
                        providerLocalData.provider + ', id: ' + providerLocalData.id,
                        result,
                        requestId
                    )
                    this.mockGetByIdRequest(provider)
    ...
    private static loadCache(id: string): any {
        ...
        return JSON.parse(readFileSync(this.cacheFolderName + id + '.json', { encoding: 'utf8' })).response
    }
}
