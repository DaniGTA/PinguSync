...
describe('AnimeOfflineDatabaseManager | basic test', () => {
    ...
    describe('getMappingFromProviderLocalData tests', () => {
        const database: AnimeOfflineDatabase = JSON.parse(
            readFileSync('./test/api/mapping-providers/anime-offline-database/data/anime-offline-database.json', {
                encoding: 'utf8',
            })
        ) as AnimeOfflineDatabase
        beforeAll(() => {
