...
describe('Provider: AniDB | Offline Test runs', () => {
    ...
    describe('search by name tests', () => {
        ...
        test('should find id 5544', async () => {
            ...
            series.addListProvider(lpdld)
            // tslint:disable-next-line: no-string-literal
            const result = await new DownloadProviderLocalDataWithoutId(series, a)['getProviderLocalDataByName'](
                new Name('Persona Trinity Soul', 'x-japclean')
            )
        ...
        test('should find id 11673 or nothing', async () => {
            ...
            series.addListProvider(lpdld)
            const result = await a['getSameTitle'](
                'Gakusen Toshi Asterisk 2',
                AniDBHelper.anidbNameManager.data as any,
                2
            )
