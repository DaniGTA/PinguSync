...
describe('Hunter x Hunter | Testrun', () => {
    beforeAll(() => {
    ...
    test('should process the anime hunter x hunter right (anilist source)', async () => {
        const series = new Series()
        const provider = new ListProviderLocalData(11061, AniListProvider)
        ...
        const resultSeries = MainListSearcher.findAllSeriesByProvider(11061, anilistName)
    ...
    /**
    test('should process the anime hunter x hunter right (Trakt source)', async () => {
        const series = new Series()
